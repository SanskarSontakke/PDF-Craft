...
export function RemoveAnnotationsTool({ className = '' }: RemoveAnnotationsToolProps) {
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      ...
      {status === 'complete' && result && <div className="p-4 rounded bg-green-900/20 border border-green-800 text-green-200"><p className="text-sm font-medium">Annotations removed successfully!</p></div>}
    </div>
  );
}
